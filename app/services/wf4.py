...
def build_report_structured(wf3_analysis: dict[str, object]) -> dict[str, object]:
    ...
    rapport = {
        ...
        "points_valides": [item.get("libelle", "") for item in valid_items[:8]],
        "points_a_confirmer": [item.get("libelle", "") for item in confirm_items[:8]],
        "points_bloquants": [item.get("libelle", "") for item in missing_items[:8]],
        "pieces_manquantes": [
        ...
        "recommandations": [item.get("action_requise", "") for item in missing_items[:10]],
    }
    return rapport
